...
def parse_file(path: Path, country: str, panel: int) -> tuple[list[dict], list[str]]:
    ...
    try:
        ...
        for row_number, row in enumerate(general.iter_rows(min_row=general_header_row + 1, values_only=True),
            ...
            support_unambiguous = len(support_rows) == 1
            ...
            source_rows.append({
                ...
                "asistio": attended,
                "puntaje_capitan": number(cell(row, general_columns, "capitan")),
                "puntaje_jurado1": number(cell(row, general_columns, "jurado 1")),
                "puntaje_jurado2": number(cell(row, general_columns, "jurado 2")),
                "total": total,
                "puntaje_pct": number(cell(row, general_columns, "puntaje")),
                "support_unambiguous": support_unambiguous,
        ...
        if fatal_blocks:
            # La asistencia del archivo no es confiable, pero sus puntajes siguen
            # siendo respuestas independientes y se conservan tal como vienen.
            for source in source_rows:
                source["asistio"] = None
        else:
            # Para archivos validados, mantener la semántica previa: una no
            # asistencia no genera puntajes de entrevista.
            for source in source_rows:
                if not source["support_unambiguous"]:
                    source["omit"] = True
                if source["asistio"] is False:
                    for field in ("puntaje_capitan", "puntaje_jurado1", "puntaje_jurado2", "puntaje_pct"):
                        source[field] = None
        source_rows = [source for source in source_rows if not source.pop("omit", False)]
        for source in source_rows:
            source.pop("support_unambiguous", None)
        if indeterminate:
...
def write_duplicate_report(by_key: dict[tuple[str, str], list[dict]]) -> None:
    ...
    with DUPLICATES.open("w", encoding="utf-8", newline="") as handle:
        ...
        for key, candidates in sorted(by_key.items()):
            ...
            if known:
                winner = max(known, key=lambda item: (item["asistio"],
                                                       item["puntaje_pct"] is not None,
                                                       item["puntaje_pct"] or -1,
                                                       item["archivo"], -item["fila"]))
                criterion = "asistencia conocida > sin dato; asistió > puntaje_pct"
            else:
                with_score = [x for x in candidates if x["puntaje_pct"] is not None]
                winner = (max(with_score, key=lambda item: item["puntaje_pct"])
                          if with_score else min(candidates, key=lambda item: (item["archivo"], item["fila"])))
                criterion = "puntaje_pct más alto" if with_score else "archivo y fila menores"
...
def main() -> int:
    ...
    print(f"Matchean con asistencia conocida: {sum(row['asistio'] is not None for row in matched)}")
    print(f"Matchean con asistencia sin dato: {sum(row['asistio'] is None for row in matched)}")
