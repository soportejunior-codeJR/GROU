...
function Explorador({ ds }: { ds: Dataset }) {
  ...
  return <><Encabezado ds={ds} totalVigente={indices.length} seleccionadas={selected} exportar={exportar} />{ds.es_ejemplo && <p style={{ color: '#8e3431' }}>Advertencia: dataset de ejemplo.</p>}<div className="toolbar"><button onClick={() => { setFiltros({}); setModo('TODAS'); }} style={secondary}>Limpiar filtros</button><label>Modo <select value={modo} onChange={(e) => setModo(e.target.value as Modo)}><option>TODAS</option><option>AL_MENOS_UNA</option></select></label><span className="muted">{Object.keys(filtros).length} filtros activos</span></div>{GROUPS.map(([name, fields]) => <section key={name}><h2>{name}</h2><div className="filtergrid">{fields.filter((c) => ds.campos[c]).map((c) => <FiltroCard key={c} ds={ds} campo={c} filtros={filtros} cambiar={cambiar} modo={modo} />)}</div></section>)}{extra.length > 0 && <section><h2>Otros campos</h2><div className="filtergrid">{(todos ? extra : extra.slice(0, 8)).map((c) => <FiltroCard key={c} ds={ds} campo={c} filtros={filtros} cambiar={cambiar} modo={modo} />)}</div>{extra.length > 8 && <button onClick={() => setTodos(!todos)} style={secondary}>{todos ? 'Mostrar menos' : `Mostrar los ${extra.length - 8} restantes`}</button>}</section>}<TablaResultados ds={ds} indices={indices} /></>;
}
